...
const ClientDashboard: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const [user, setUser] = useState<User>({ firstName: 'العميل', email: '', uid: '', role: 'client' });
        ...
        <View style={styles.section}>
          ...
          {jobs.slice(0, 3).map((job) => (
            <View key={job.id} style={styles.jobCard}>
              ...
              <Text style={styles.jobDetails}>Area: {job.area} m² • Date: {job.date}</Text>
...
const ProviderDashboard: React.FC<{ navigation: any }> = ({ navigation }) => {
  const [jobs, setJobs] = useState<Job[]>([]);
  const [user, setUser] = useState<User>({ firstName: 'مقدم الخدمة', email: '', uid: '', role: 'provider' });
  const [activeTab, setActiveTab] = useState<'Home' | 'Jobs' | 'Track' | 'Profile'>('Home');
